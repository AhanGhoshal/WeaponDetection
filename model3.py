...
from keras.api.layers import GlobalAveragePooling2D, Dense, Dropout, Input
...
def create_model(no_of_classes):
    # Load ResNet50 as the base model, pre-trained on ImageNet, excluding the top layers
    base_model = ResNet50V2(input_tensor=Input(shape=(image_size, image_size, 3)),
                            include_top=False,
                            weights='imagenet')

    # Unfreeze some layers of ResNet50 to allow fine-tuning
    ...
    x = Dense(32, activation='relu')(x)  # Another dense layer
    class_prediction = Dense(no_of_classes, activation='softmax', name='class_output')(x)  # Output layer for classes

    # Box output layer (Bounding box prediction)
    box_output = Dense(256, activation='relu')(x)
    ...
    box_predictions = Dense(4, activation='sigmoid', name='box_output')(box_output)  # Output layer for boxes

    # Create the model with inputs from ResNet50 and outputs for class and box predictions
    model = models.Model(inputs=base_model.input, outputs=[box_predictions, class_prediction])

    return model

# Define the number of classes (for binary classification: knife vs. no weapon)
model = create_model(no_of_classes=2)
...
losses = {
    'box_output': 'mean_absolute_error',  # Changed to MAE for bounding boxes
    'class_output': 'sparse_categorical_crossentropy'  # Binary crossentropy can be used if only 2 classes
}
loss_weights = {
    'box_output': 1.0,
    'class_output': 1.0
...
metrics = {
    ...
    'box_output': 'mae'  # Mean absolute error for bounding boxes
}

# Setup callbacks
stop = tf.keras.callbacks.EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=40, restore_best_weights=True)
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.0002, patience=30, min_lr=1e-7, verbose=1)

# Compile the model
opt = SGD(learning_rate=1e-4, momentum=0.9)  # Lowered learning rate for fine-tuning
model.compile(optimizer=opt, loss=losses, loss_weights=loss_weights, metrics=metrics)

# Fit the model (assuming you have `train_images`, `train_labels`, `train_boxes`, `val_images`, etc.)
...
model.save('WeaponPrediction_resnet50_finetuned.h5')
