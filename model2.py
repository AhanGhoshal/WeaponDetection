...
def extract_xml_contents(annot_directory, image_dir, target_size=224):
...
def oversample_data(images, labels, boxes):
    # Separate the classes
    images = np.array(images)
    boxes = np.array(boxes)
    labels = np.array(labels)

    class_0_indices = np.where(labels == 0)[0]  # Indices for no weapon
    class_1_indices = np.where(labels == 1)[0]  # Indices for knife

    # Resample class 0 (no weapon) to match class 1
    class_0_images, class_0_labels, class_0_boxes = images[class_0_indices], labels[class_0_indices], boxes[class_0_indices]
    class_1_images, class_1_labels, class_1_boxes = images[class_1_indices], labels[class_1_indices], boxes[class_1_indices]

    class_0_images_resampled, class_0_labels_resampled, class_0_boxes_resampled = resample(
        class_0_images, class_0_labels, class_0_boxes,
        replace=True, n_samples=len(class_1_indices), random_state=42
    )

    # Combine the resampled data
    combined_images = np.vstack((class_0_images_resampled, class_1_images))
    combined_labels = np.hstack((class_0_labels_resampled, class_1_labels))
    combined_boxes = np.vstack((class_0_boxes_resampled, class_1_boxes))

    # Shuffle the data
    shuffled_indices = np.random.permutation(len(combined_images))
    return combined_images[shuffled_indices], combined_labels[shuffled_indices], combined_boxes[shuffled_indices]

train_images, train_labels, train_boxes = oversample_data(train_images, train_labels, train_boxes)
...
def augment_minority_class(images, labels, boxes, class_to_augment=0, augmentation_factor=2):
    # Separate the minority class
    minority_indices = np.where(labels == class_to_augment)[0]
    minority_images = images[minority_indices]
    minority_labels = labels[minority_indices]
    minority_boxes = boxes[minority_indices]
    ...
    for img, lbl, box in zip(minority_images, minority_labels, minority_boxes):
    ...
    boxes = np.vstack((boxes, np.array(augmented_boxes)))

    # Shuffle the data
    shuffled_indices = np.random.permutation(len(images))
    return images[shuffled_indices], labels[shuffled_indices], boxes[shuffled_indices]

# Apply data augmentation
train_images, train_labels, train_boxes = augment_minority_class(train_images, train_labels, train_boxes, class_to_augment=0, augmentation_factor=2)
...
model.save("WeaponPrediction_new.h5")
